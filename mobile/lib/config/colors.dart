import 'package:flutter/material.dart';

class WTColor {
  static Color get transparent => Colors.transparent;
  static Color get white => Colors.white;
  static Color get black => Colors.black;
  static Color get text => Colors.black87;
  static Color get textWhite => Colors.white;
  static Color get textGray => const Color.fromARGB(170, 0, 0, 0);
  static Color get background => const Color.fromARGB(255, 247, 247, 247);
  static Color get error => Colors.redAccent;
  static Color get success => Colors.greenAccent;
  static Color get shimmer => const Color.fromARGB(255, 202, 202, 202);

  static Color get mainGreen => const Color.fromARGB(255, 75, 159, 7);
}
